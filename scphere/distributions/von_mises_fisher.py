...
import torch.distributions.von_mises
...
def kl_vmf_hyu(vmf, hyu):
    # print("do we enter this KL divergence?")
    # print(vmf.loc.shape[-1])
    # print(hyu.dim)
    # so does vmf.loc.shape[-1] always have to be 1 greater than hyu's dim?
    # e.g. if shape is 3 (3D), then hyu must be in 2D?
    torch._assert(vmf.loc.shape[-1] - 1 == hyu.dim, "not equal")
